...
M3 通用编辑器和 M4 编辑历史与效率功能已经完成。编辑器可以通过 `GameDataArchive` 打开 `CommonData.dat`，识别其中 39 项配置，在动态表格中管理记录，并通过属性面板编辑常用标量、枚举和集合；属性修改、记录增删复制、剪切粘贴和批量修改均可撤销和重做。现已支持跨配置全文搜索与结果跳转、窗口和表格筛选状态恢复，以及打开、搜索、记录管理等常用键盘快捷键。后续开发以 [docs/TASKS.md](docs/TASKS.md) 为唯一任务台账。
