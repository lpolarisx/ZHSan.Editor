...
M1 配置发现与项目工作区、M2 档案加载与安全保存、M3 通用编辑器、M4 编辑历史与效率功能、M5 校验与引用关系，以及 M6 导入、导出与发布已经完成；M7 已完成专用配置编辑器扩展机制、科技树编辑器和设施种类/等级组合编辑器。编辑器可以通过 `GameDataArchive` 打开 `CommonData.dat`，识别其中 39 项配置，在动态表格中管理记录，并通过属性面板编辑常用标量、枚举、集合和已声明的跨表引用；属性修改、记录增删复制、剪切粘贴、批量修改和导入均可撤销和重做。保存链路支持临时文件、备份、内容指纹和外部变更冲突保护，并已验证现有 JSON 与游戏 API 的双向兼容。现已支持最近项目、关闭与退出前的未保存确认、跨配置全文搜索与结果跳转、窗口和表格筛选状态恢复、常用键盘快捷键，以及完整的字段、单表和跨表校验。数据交换支持单配置 JSON 导入导出、游戏档案批量导入、全项目目录导出、三种导入合并策略、提交前差异与失败预览及持久化操作日志；严格校验通过后还可生成独立、重新读取验证过的正式 `CommonData.dat` 发布档案。科技配置可在可缩放、可搜索的二维关系图中编辑节点位置及前后置链路，自动维护双向关系并阻止新增循环；设施等级配置可按设施种类分组管理、自动创建下一级并直观识别组合冲突；专用编辑面仍可随时切回通用表格。后续开发以 [docs/TASKS.md](docs/TASKS.md) 为唯一任务台账。
