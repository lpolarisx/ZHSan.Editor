...
导入策略包括整表替换、按 ID 合并和仅添加新 ID。应用层先生成不修改当前文档的 `ConfigMergePlan`：整表替换保持导入顺序，按 ID 合并保留当前顺序并追加新 ID，仅新增忽略已有 ID 的修改。无 ID 配置仅支持整表替换，重复 ID 以显式冲突阻止有歧义的合并。导入提交前展示新增、修改、删除及冲突数量。
...
- 当前已实现档案加载、安全保存、外部变更冲突保护、另存为、保存副本、通用记录编辑器、撤销历史及三层校验框架，并已支持必填、数值范围、固定数组长度、单表 ID 唯一性、显式跨表引用索引、目标存在性、删除影响确认、科技前后置一致性与循环依赖校验，以及校验结果筛选、定位和保存/发布预检策略；导入基础设施已具备差异模型和整表替换、按 ID 合并、仅新增三种合并计划。
