...
导出前执行与保存一致的完整项目校验并展示结果，但允许用户导出尚未完成的工作数据。单配置 JSON 和全项目目录导出均复用 `GameDataArchive` 的序列化器：先生成临时档案条目，再将条目原子写入目标 JSON，确保格式与游戏 API 一致。目录导出不清理无关文件，逐配置记录成功或失败并继续处理其余配置，同时拒绝越过所选目录的条目路径。导入和导出共享最多 500 条的本地持久化传输日志。
...
- 当前已实现档案加载、安全保存、外部变更冲突保护、另存为、保存副本、通用记录编辑器、撤销历史及三层校验框架，并已支持必填、数值范围、固定数组长度、单表 ID 唯一性、显式跨表引用索引、目标存在性、删除影响确认、科技前后置一致性与循环依赖校验，以及校验结果筛选、定位和保存/发布预检策略；导入导出链路已支持单配置 JSON、游戏档案批量导入、三种合并策略、提交前差异与失败预览、可撤销应用、单配置及全项目目录导出和持久化传输日志。
