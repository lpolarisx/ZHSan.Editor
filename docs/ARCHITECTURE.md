...
- 当前已实现档案加载、安全保存、外部变更冲突保护、另存为、保存副本、通用记录编辑器、撤销历史及三层校验框架，并已支持必填、数值范围、固定数组长度、单表 ID 唯一性、显式跨表引用索引、目标存在性、删除影响确认、科技前后置一致性与循环依赖校验，以及校验结果筛选、定位和保存/发布预检策略；M6 导入、导出与发布链路已完整支持单配置 JSON、游戏档案批量导入、三种合并策略、提交前差异与失败预览、可撤销应用、单配置及全项目目录导出、严格校验发布和持久化操作日志。
